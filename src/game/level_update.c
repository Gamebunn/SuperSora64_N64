...
void init_mario_after_warp(void) {
    ...
    if (gCurrDemoInput == NULL) {
        ...
        if (gCurrLevelNum == LEVEL_BOB
            && get_current_background_music() != SEQUENCE_ARGS(4, SEQ_VSMETAL) && sTimerRunning) {
            play_music(SEQ_PLAYER_LEVEL, SEQUENCE_ARGS(4, SEQ_VSMETAL), 0);
        }
#endif
...
s16 music_changed_through_warp(s16 arg) {
    ...
    if (levelNum == LEVEL_BOB && levelNum == gCurrLevelNum && destArea == gCurrAreaIndex) {
        ...
        if (sp2C == SEQUENCE_ARGS(4, SEQ_SUPER | SEQ_VARIATION)
            || sp2C == SEQUENCE_ARGS(4, SEQ_SUPER)) {
            val4 = 0;
        }
    } else {
